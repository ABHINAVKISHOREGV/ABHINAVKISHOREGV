...
alias cp='cp -i'
alias mv='mv -i'
if [[ \"$(kubectl config current-context)\" == 'dev' ]];then kubectl config set-context --current --namespace=protons; fi  
alias k='kubectl' # Use k instead of kubectl
""" > ${HOME}/k8sconfig/rently-$ENV/.bashrc
	fi
	cp -r "${HOME}/.kube" "${HOME}/k8sconfig/rently-$ENV/.kube"
	docker run -it -h "rently-$ENV" --name "rently-$ENV"  -v "${HOME}/k8sconfig/rently-$ENV":/root --rm abhinavkishoregv/kctl-debian:latest bash -c "echo 'Use Kubectl commands just like your machine. You can also use the below aliases'; tail -n 1 ~/.bashrc && cd && bash"
fi
